...
# ---------- Belge no desenleri ----------
DOCNO_RE = re.compile(r"\bE(?:AR|FR)\d{13}\b", re.I)   # EAR2025..., EFR2025...
...
class Parsed:
    def __init__(self):
        ...
        self.lines: List[Dict[str,Any]] = []  # blob, unit_price, line_total, qty, kdv
        self.inv_kdv: Optional[float] = None  # fatura genel KDV modu
        self.text_upper: str = ""             # notlar+satır adları birleşik

def _line_kdv_percent(line: ET.Element) -> Optional[float]:
    # 1) TaxTotal/TaxSubtotal/Percent
    p1 = xfind(line, "cac:TaxTotal/cac:TaxSubtotal/cbc:Percent")
    if p1 is not None and norm(p1.text):
        f = _to_float(p1.text)
        if f is not None: return f
    # 2) ClassifiedTaxCategory/Percent
    p2 = xfind(line, "cac:Item/cac:ClassifiedTaxCategory/cbc:Percent")
    if p2 is not None and norm(p2.text):
        f = _to_float(p2.text)
        if f is not None: return f
    # 3) Genel Percent araması
...
def parse_invoice_xml(xml_bytes: bytes) -> Parsed:
    ...
    for line in root.findall("cac:InvoiceLine", NS):
        ...
        P.lines.append({
            "blob": blob,
            "unit_price": xtext(line, "cac:Price/cbc:PriceAmount"),
            "line_total": xtext(line, "cac:LineExtensionAmount"),
            "qty":        xtext(line, "cbc:InvoicedQuantity"),
            "kdv":        kdv
        })
        P.items.append(blob)
